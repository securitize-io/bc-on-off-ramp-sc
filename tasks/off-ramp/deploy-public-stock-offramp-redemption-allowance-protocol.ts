...
/*
npx hardhat deploy-public-stock-offramp-allowance-protocol \
    --network sepolia \
    ...
    --verify
*/
task('deploy-public-stock-offramp-allowance-protocol', 'Deploy Public Stock Off-Ramp Protocol (Allowance implementation)')
    ...
    .setAction(async (args, hre) => {
        if (!args.silenceLogs) {
            consoleCyan('\n task: deploy-public-stock-offramp-allowance-protocol');
        }
        ...
        if (!args.silenceLogs) {
            console.log('Successfully updated liquidity provider on public stock redemption contract');
...
// Deploy PublicStockOffRamp proxy
// npx hardhat deploy-public-stock-offramp --network sepolia --asset 0x123 --nav-provider 0xe76B92272667363FD487a71c13b7799ED924C9b8 --fee-manager 0xe76B92272667363FD487a71c13b7799ED924C9b8 --asset-burn false --verify
task('deploy-public-stock-offramp', 'Deploy PublicStockOffRamp proxy')
    .addParam('asset', 'DS Token to be redeemed')
    .addParam('navProvider', 'NAV rate provider address')
    .addParam('feeManager', 'Fee manager address')
    .addParam('assetBurn', 'Whether assets should be burned on redemption')
    ...
    .setAction(async (taskArgs, hre) => {
        if (!taskArgs.silenceLogs) {
            consoleCyan('\n task: deploy-public-stock-offramp');
        ...
        const { proxyAddress, implAddress } = await hre.run('deploy-proxy', {
            contractName: 'PublicStockOffRamp',
            kind: 'uups',
